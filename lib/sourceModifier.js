...
const proMergeRegex = /for.*(const.*exchange.*in.*pro.*\{[\s\S]+?^\})/gm
 ...
 * @param {boolean} removePro - if should remove pro entirely
 */
function filterExchanges(source, exchangeList, removePro) {
  ...
  if (removePro) {
    updatedSource = updatedSource.replace(proMergeRegex, '')
    updatedSource = updatedSource.replace(proRegex, '')
    let lines = updatedSource.split('\n')
    lines = lines.filter((line) => !line.includes('pro'))
    lines.push(
